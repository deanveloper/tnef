...
func TestAttachments(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.in, func(t *testing.T) {
			out, err := Decode(test.Read(t, "./testfiles", tc.in+".tnef"))
			if err != nil {
				t.Fatal(err)
			}

			if len(out.Attachments) != len(tc.wantAttachments) {
				t.Errorf("wrong length; want %v, got %v",
					len(tc.wantAttachments), len(out.Attachments))
			}
			...
			for _, want := range tc.wantAttachments {
...
func TestBadChecksum(t *testing.T) {
	...
	if err == nil {
		t.Error("this should fail because of a bad checksum")
	}
}
